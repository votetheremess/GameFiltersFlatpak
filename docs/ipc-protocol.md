...
- **Filesystem path**: `$XDG_RUNTIME_DIR/lumen.sock`
- **Linux abstract namespace**: `@lumen.sock`

Both use the same name — a protocol invariant referenced as `lumen::kSocketName` (`layer/src/ipc.hpp`) on the C++ side and `SOCKET_NAME` (`frontend/src/ipc.rs`) on the Rust side. The abstract socket exists so Steam pressure-vessel / bubblewrap-sandboxed processes — which cannot see the host's `$XDG_RUNTIME_DIR` — can still reach the frontend. The layer tries both; either succeeds is enough.
